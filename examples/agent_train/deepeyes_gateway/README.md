...
- Run from the Uni-Agent repository with the `verl` trainer dependencies
  available.
- Install DeepEyes example dependencies: `httpx`, `Pillow`, `requests`, and
  `openai`.
...
- The image tool implementation is loaded by the standard VERL native tool
  registry from this example package, so the recipe exercises the same runtime
  tool stack as existing tool-use training while keeping the example
  self-contained.
